...
export const spacesApi = {
  ...
  async updateDoc(id: string, data: { title?: string; docDate?: string; teamId?: string | null; assigneeRoleRestrictions?: string[] }): Promise<{ doc: Doc }> {
  ...
  async updateList(id: string, data: { name?: string; customGroups?: string[]; applyToAll?: boolean }): Promise<{ list: any }> {
    return apiClient<{ list: any }>(`/api/lists/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    });
